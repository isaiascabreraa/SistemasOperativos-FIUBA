...
{
	if (arg[0] == '$') {
		char *var_name = arg + 1;  // Saltar el primer carácter '$'
		char *var_value;

		if (*var_name == '?') {  // Chequea si es la pseudo-variable
			char aux[10];
		...
		// Si la variable de entorno existe, reemplazar el token con su valor
		if (var_value != NULL) {
			// Crear un nuevo token con el valor de la variable de entorno
			size_t new_size = strlen(var_value) + 1;
			...
			// Liberar la memoria del token original y devolver el nuevo token
			free(arg);
			return new_arg;
		} else {
			// Si la variable de entorno no existe, devolver un token vacío
			arg[0] = '\0';
			return arg;
		}
	} else {
		// Si el token no comienza con '$', devolver el token original
		return arg;
	}
}
...
struct cmd *
parse_line(char *buf)  // ls -l | grep Doc | wc
{
	struct cmd *r, *l;
	r = NULL;
